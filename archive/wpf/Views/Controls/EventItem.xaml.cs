...
public partial class EventItem : UserControl
{
    ...
    private static void OnEventDataChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        var displayTitle = $"{evt.Title}({datePart})";
        item.TitleText.Text = displayTitle;
        item.TitleText.ToolTip = displayTitle;

        // 设置时间文本
        item.TimeText.Text = EventListViewModel.FormatEventSummary(evt);
        item.TimeText.ToolTip = item.TimeText.Text;
    ...
    {
        base.OnMouseLeave(e);
        HoverBorder.Background = Application.Current.TryFindResource("EventCardBrush") as System.Windows.Media.Brush;
