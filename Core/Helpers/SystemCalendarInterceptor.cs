...
public class SystemCalendarInterceptor : IDisposable
{
    ...
    private static bool IsSystemCalendarWindow(IntPtr hwnd)
    {
        ...
        var taskbarEdge = WindowPositionHelper.DetectTaskbarEdge(
            new Rect(
                monitorArea.Left,
                monitorArea.Top,
        ...
        // 系统日历/通知中心窗口特征：
        // 1. 根据任务栏方向贴近工作区对应角落（允许 50px 误差，考虑不同 DPI）
        // 2. 窗口宽度 > 200px
        // 3. 窗口高度 > 100px
        var validSize = windowWidth > 200 && windowHeight > 100;
        var alignedWithTaskbar = WindowPositionHelper.IsFlyoutAlignedWithTaskbar(
            taskbarEdge,
            new Rect(rect.Left, rect.Top, windowWidth, windowHeight),
            new Rect(
                workArea.Left,
                workArea.Top,
                workArea.Right - workArea.Left,
        ...
        if (alignedWithTaskbar && validSize)
        {
