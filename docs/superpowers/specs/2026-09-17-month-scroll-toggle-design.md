...
日期：2026-09-17
状态：已批准（2026-09-17 修订：滑动方向改为固定向下）
...
- 开（默认）：保持现状，整月切换 + 淡入。
- 关：翻月改为**瀑布流式平滑下滑**动画：无论翻到下一个月还是上一个月，
  新月份始终从网格区顶部滑入、旧月份始终向底部滑出，方向固定向下。

## 决策记录

| 决策点 | 选择 | 说明 |
| --- | --- | --- |
| 动画样式 | 瀑布流式垂直下滑 | 用户指定；方向固定向下，不随翻页方向变化 |
| 滑动方向 | 固定向下 | 新月份一律自顶部滑入、旧月份一律向底部滑出；横向滑动、方向感知滑动、连续手势滚动方案已否决 |
...
### 3. 瀑布流平滑下滑动画（main.cpp）

- 新增状态：`scrollAnimating_`、`scrollProgress_`（0→1）、
  `scrollFromYear_ / scrollFromMonth_`（滑动起点月份）。
  方向固定向下，不记录方向状态。
- 时长约 260ms，ease-out cubic（`p = 1 - (1-t)^3`），复用 `kAnimationTimer`（16ms）。
- 每帧绘制两次（瀑布流：内容自上而下流动）：
  - 新月份：offsetY 从 `-kSlideHeight` → 0，自顶部滑入。
  - 旧月份：offsetY 从 0 → `+kSlideHeight`，向底部滑出。
...
- `ChangeMonth(delta)`：计算新 (year, month) 后按 `settings_.monthPaging` 分流——
  关 → `BeginScrollTransition(fromYear, fromMonth)`；开 → 现有 `BeginTransition()`。
  动画方向固定向下，无需区分前后。
- `ChangeYear(delta)`：同样分流（年切换 = 一次瀑布流下滑，只画起点月与终点月）。
...
- 「常规」页「每周开始」下方新增一行：
  - 标签「整月翻滚」（y ≈ 352）+ 说明「关闭后翻月时瀑布流平滑下滑」；
...
- 根 README 功能清单补一句：翻月支持整月切换或瀑布流平滑下滑（设置中切换）。
