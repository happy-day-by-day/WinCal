...
| 决策点 | 选择 | 说明 |
| --- | --- | --- |
| 动画样式 | 垂直滑动 | 用户指定；横向滑动、连续手势滚动方案已否决 |
| 滑动方向 | 按方向 | 下一个月 = 网格向上滚动（新月份自底部滑入）；上一个月相反 |
...
- 新增状态：`scrollAnimating_`、`scrollProgress_`（0→1）、`scrollDir_`
  （+1 = 下一个月，网格向上滚）、`scrollFromYear_ / scrollFromMonth_`（滑动起点月份）。
- 时长约 260ms，ease-out cubic（`p = 1 - (1-t)^3`），复用 `kAnimationTimer`（16ms）。
- 每帧绘制两次：
  - 旧月份：offsetY 从 0 → `-kSlideHeight * dir`，滑出。
  - 新月份：offsetY 从 `+kSlideHeight * dir` → 0，滑入。
...
- `ChangeMonth(delta)`：计算新 (year, month) 后按 `settings_.monthPaging` 分流——
  关 → `BeginScrollTransition(dir, fromYear, fromMonth)`；开 → 现有 `BeginTransition()`。
- `ChangeYear(delta)`：同样分流（年切换 = 一次同方向单格滑动，只画起点月与终点月；
  方向约定与月份一致——下一年按「下一个月」的方向滑动，上一年相反）。
- 点击相邻月日期（`WM_LBUTTONUP` 中 `selected_` 与显示月不同步的分支）：同样分流，
  方向按 `目标年×12 + 目标月` 与 `当前显示年×12 + 当前显示月` 的差值符号判定
  （跨年如 12 月 → 1 月为正方向）。
...
- 根 README 功能清单补一句：翻月支持整月切换或垂直平滑滚动（设置中切换）。
